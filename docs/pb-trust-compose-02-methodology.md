...
For N boundaries, a request is decoded and re-encoded by N worker processes before final authorization/outcome evaluation. Worker startup happens before the timed region. Process scheduling, encode/decode, IPC system calls, and final validation are inside the measured wall-clock path.
...
v0.1 uses a fixed 42-byte network-order record containing:
...
- mean payload bytes;
- logical request wire bytes across process boundaries;
...
5. Unsupported physical interpretations are not inferred from Python IPC timing.
6. Negative results must remain in the scorecard.
...
If JSON is slower but compact software and ProofBit are similar, the result belongs to encoding choice rather than proof-native semantics.
